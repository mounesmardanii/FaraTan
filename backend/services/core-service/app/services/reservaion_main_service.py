...
class ReservationMainService:
    ...
    async def reserve_session(self, member_id: UUID, session_id: UUID) -> ReservationResponseSchema:
        print(session_id)
        ...
        if self.reservation_service.already_reserved(member_id, session_id):
            raise HTTPException(status_code=400, detail="Already reserved")

        await self.session_service.reduce_capacity(session.id)

        created = self.reservation_service.create(member_id, session_id)
    ...
    async def get_my_reservations(self, member_id: UUID) -> List[ReservationResponseSchema]:
        reservations = self.reservation_service.get_by_member_id(member_id)
