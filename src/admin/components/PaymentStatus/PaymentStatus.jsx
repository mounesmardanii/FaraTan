...
const PaymentStatus = () => {
    ...
    const [payments, setPayments] = useState([
        { id: 1, name: 'مونس مردانی', amount: 150, date: '1 آذر', method: 'آنلاین', course: 'یوگا', status: 'پرداخت شده' },
        { id: 2, name: 'ملاحت مردانی', amount: 200, date: '5 آذر', method: 'کارتخوان', course: 'پیلاتس', status: 'پرداخت شده' },
        { id: 3, name: 'غزل نادری', amount: 0, date: '', method: '', course: '', status: 'پرداخت نشده' },
        { id: 4, name: 'نگین شاکری', amount: 400, date: '10 آذر', method: 'نقدی', course: 'تی آر ایکس', status: 'پرداخت شده' }
    ]);
    ...
    const handleAdd = () => {
        ...
        setNewPayment({ name: '', amount: '', date: '', method: '', course: '', status: '' });
    };
            ...
            <div className="flex flex-1 relative mt-4">
                ...
                >
                    <motion.input
                        ...
                        className="border-2 border-[#9FC6C3] rounded-full 
             px-2 py-1 text-[11px] 
             sm:px-3 sm:py-2 sm:text-[13px] 
             md:text-sm 
             w-full max-w-[80%] sm:max-w-[75%] md:max-w-sm 
             mx-auto text-right text-[#055B5C] focus:outline-none"
                        value={search}
                                ...
                                <table className="w-full text-xs md:text-sm text-[#055B5C] text-center font-semibold">
                                    ...
                                    <tbody>
                                        ...
                                        <tr className="bg-[#EAF4EF]">
                                            {['name', 'amount', 'date', 'method', 'course', 'status'].map((key) => (
                            ...
                            <div className="md:hidden flex flex-col gap-4">
                                {filtered.map((p, i) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between mb-1">
                                            <span className="font-bold">دوره:</span>
                                            <span>{p.course || '-'}</span>
                                        </div>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            <button
                                                className="bg-[#FF6600] text-white px-2 py-1 rounded text-xs hover:bg-[#e65c00] transition cursor-pointer"
                                                onClick={() => handleEdit(p.id)}
                                            >
                                                ویرایش
                                            </button>
                                            <button
                                                className="bg-[#9FC6C3] text-[#055B5C] px-2 py-1 rounded text-xs hover:bg-[#85bab4] transition cursor-pointer"
                                                onClick={() => handleDelete(p.id)}
                                            >
                                                حذف
                                            </button>
